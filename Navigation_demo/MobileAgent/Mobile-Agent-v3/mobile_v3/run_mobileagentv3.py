...
def run_instruction(adb_path, hdc_path, api_key, base_url, model, instruction, add_info, coor_type, if_notetaker, screenrecord=False, recording_dir=None, max_step=25, log_path="./logs"):
    ...
    if screenrecord:
        if recording_dir:
            ...
            sanitized_instr = ''.join(c if c.isalnum() or c in ('-', '_') else '_' for c in instruction[:10])
            video_name = f"{time_str}_{sanitized_instr}_session.mp4"
            recording_path = os.path.join(recording_dir, video_name)
        else:
            default_recording_dir = os.path.join(save_path, "recordings")
            os.mkdir(default_recording_dir)
            recording_path = os.path.join(default_recording_dir, "session.mp4")
        controller.start_recording(recording_path)

    def save_task_result(hit_step_limit):
        task_result_path = os.path.join(save_path, "task_result.json")
        current_time = datetime.now()
        formatted_time = current_time.strftime("%Y-%m-%d %H:%M:%S.%f")
        task_result_data = {"goal": instruction, "finish_dtime": formatted_time, "hit_step_limit": hit_step_limit}
        if recording_path:
            task_result_data["recording_path"] = recording_path
...
if __name__ == '__main__':
    ...
    parser.add_argument("--recording_dir", type=str, default=None, help="Optional directory to store recorded videos when --screenrecord is enabled.")
    args = parser.parse_args()
    
    run_instruction(args.adb_path, args.hdc_path, args.api_key, args.base_url, args.model, args.instruction, args.add_info, args.coor_type, args.notetaker, args.screenrecord, args.recording_dir)
